...
{
	ZeroMemory(prop, sizeof(float) * 4);
}
...
int Drone::prepare_body()
{
	...
	masses.push_back({ Vector3(0.008f, -0.001f, 0.007f) , 0.01f });

	body->body_init(this, masses, Vector3(), Vector3(), Vector3(), 0.f, Vector3());
	return 0;
}

void Drone::interaction(ForceList& forces, Body* body)
{
	forces.emplace_back(Vector3(0.f, 0.f, 0.f), Vector3(0.f, 0.f, -9.8f*body->get_mass()));
	...
	for (int i = 0; i < 4; i++)
	{
		...
		forces.emplace_back(temp_motorpos, prop_dir * prop[i]);
	}

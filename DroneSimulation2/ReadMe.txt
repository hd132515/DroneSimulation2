================================================================================
    MFC 라이브러리 : DroneSimulation2 프로젝트 개요
===============================================================================

응용 프로그램 마법사에서 이 DroneSimulation2 응용 프로그램을 만들었습니다. 이 응용 프로그램은 MFC의 기본 사용법을 보여 줄 뿐만 아니라 응용 프로그램을 작성하기 위한 기본 구조를 제공합니다.

DroneSimulation2 응용 프로그램을 구성하는 각 파일에 대한
요약 설명이 포함되어 있습니다.

DroneSimulation2.vcxproj
...
DroneSimulation2.vcxproj.filters
...
DroneSimulation2.h
    ...
    여기에는 resource.h를 비롯한 다른 프로젝트별 헤더가 포함되어 있으며 CDroneSimulation2App 응용 프로그램 클래스가 선언되어 있습니다.

DroneSimulation2.cpp
    이는 응용 프로그램 클래스 CDroneSimulation2App가 포함된 기본 응용 프로그램 소스 파일입니다.
...
res\DroneSimulation2.ico
    아이콘 파일이며, 응용 프로그램의 아이콘으로 사용됩니다. 이 아이콘은 기본 리소스 파일인 DroneSimulation2.rc에 의해 포함됩니다.

res\DroneSimulation2.rc2
    이 파일에는 Microsoft Visual C++ 이외의 다른 도구에서 편집한 리소스가 포함되어 있습니다. 리소스 편집기로 편집할 수 없는 모든 리소스는 이 파일에 넣어야 합니다.

DroneSimulation2.reg
...
res\Toolbar.bmp
    ...
    초기 도구 모음과 상태 표시줄은 CMainFrame 클래스에서 생성됩니다. 이 도구 모음 비트맵을 리소스 편집기로 편집하고, DroneSimulation2.rc의 IDR_MAINFRAME TOOLBAR 배열을 업데이트하여 도구 모음 단추를 추가합니다.
/////////////////////////////////////////////////////////////////////////////

응용 프로그램 마법사에서 문서 형식과 뷰를 한 개씩 만듭니다.

DroneSimulation2Doc.h, DroneSimulation2Doc.cpp - 문서
    이 파일에는 CDroneSimulation2Doc 클래스가 포함됩니다. 이 파일을 편집하여 특수 문서 데이터를 추가하고 CDroneSimulation2Doc::Serialize를 통해 파일 저장 및 로드를 구현합니다.
    문서에는 다음과 같은 문자열이 있습니다.
        파일 확장명:      sc
        파일 형식 ID:        DroneSimulation2.Document
        주 프레임 캡션:  DroneSimulation2
        Doc 형식 이름:       DroneSimulation
        필터 이름:         DroneSimulation2 Files (*.sc)
        파일의 새 약식 이름: DroneSimulation
        파일 형식의 긴 이름: DroneSimulation2.Document

DroneSimulation2View.h, DroneSimulation2View.cpp - 문서 뷰
    이 파일에는 CDroneSimulation2View 클래스가 포함됩니다.
    CDroneSimulation2View 개체는 CDroneSimulation2Doc 개체를 보는 데 사용됩니다.
...
StdAfx.h, StdAfx.cpp
    이 파일은 미리 컴파일된 헤더(PCH) 파일 DroneSimulation2.pch와 미리 컴파일된 형식(PCT) 파일 StdAfx.obj를 빌드하는 데 사용됩니다.

Resource.h
    새 리소스 ID를 정의하는 표준 헤더 파일입니다. Microsoft Visual C++에서 이 파일을 읽고 업데이트합니다.

DroneSimulation2.manifest
